...
redef class AMethPropdef
	...
	do
		...
		v.unbox_signature(mpropdef, arguments)
		...
		if recv_var == null then
			...
			recv_var = v.box(recv_var, return_mtype)
			v.ret(recv_var)
		end
...
redef class MExplicitCall
	...
	do
		...
		if mproperty.is_init then
			...
			recv_var = nitni_visitor.box(recv_var, mtype)
		end
		...
		for p in msignature.mparameters do
			...
			arg = nitni_visitor.box(arg, arg_mtype)
			vars.add(arg)
		end
		...
		if return_mtype != null then
			...
			ret_var = nitni_visitor.unbox(ret_var, return_mtype)
...
redef class MExplicitSuper
	...
	do
		...
		recv_var = nitni_visitor.box(recv_var, mclass_type)
		vars.add(recv_var)
...
redef class MExplicitCast
	...
	do
		...
		from_var = nitni_visitor.box(from_var, from)
		...
		recv_var = nitni_visitor.unbox(recv_var, to)

...
redef class AExternPropdef
	...
	do
		...
		for mtype in foreign_callbacks.types do
			if not amodule.foreign_callbacks.types.has(mtype) then
				if mtype.is_cprimitive then
					assert mtype isa MClassType
					#mtype.compile_nitni_type(v, ccu) TODO separate compilation of extern types
				else
					mtype.compile_extern_type(v, ccu)
					if amodule.check_callback_compilation(mtype) then
						mtype.compile_extern_helper_functions(v, ccu)
					end
				end
			end
		end
...
redef class AExternMethPropdef
	...
	do
		...
		var externname = mpropdef.mproperty.build_cname(mpropdef.mclassdef.bound_mtype, mmodule, "___impl", long_signature, internal_call_context)
...
redef class AbstractCompilerVisitor
	fun var_from_c(name: String, mtype: MType): RuntimeVariable
	do
		if mtype.is_cprimitive then
	...
	fun ret_to_c(src: RuntimeVariable, mtype: MType)
	do
		if mtype.is_cprimitive then
			add("return {src};")
		else
...
redef class MClassType
	fun compile_nitni_type(v: AbstractCompilerVisitor, ccu: CCompilationUnit)
	do
		# define friendly type
		ccu.header_c_types.add("struct instance_{mclass.c_name} \{\n")
		ccu.header_c_types.add("const struct type *type;\n")
		ccu.header_c_types.add("const struct class *class;\n")
		ccu.header_c_types.add("{ctype} value;\n")
		ccu.header_c_types.add("\};\n")
	end
end
...
redef class MExplicitCall
	...
	do
		...
		# In nitni files, #define friendly as extern
		#var full_cname = mproperty.build_cname(recv_mtype, v.compiler.mainmodule, null, from_c_call_context)
		#var friendly_cname = mproperty.build_cname(recv_mtype, v.compiler.mainmodule, null, to_c_call_context)
		#ccu.header_decl.add("#define {friendly_cname} {full_cname}\n")
...
redef class MExplicitSuper
	...
	do
		...
		# In nitni files, #define friendly as extern
		var friendly_cname = mproperty.build_cname(mclass_type, v.compiler.mainmodule, "___super", short_signature, to_c_call_context)
		var internal_cname = mproperty.build_cname(mclass_type, v.compiler.mainmodule, "___super", long_signature, internal_call_context)
...
redef class MExplicitCast
	...
	do
		...
		var nitni_visitor = v.compiler.new_visitor
		nitni_visitor.frame = v.frame
		#var check_csig = check_csignature
		#ccu.header_decl.add("{check_csig};\n")

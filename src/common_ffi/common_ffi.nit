	...
	do
		...
		for nclassdef in n_classdefs do
			if nclassdef isa AStdClassdef and nclassdef.n_extern_code_block != null then # classes with an extern type
				...
				assert language != null
				present_languages.add(language)
				nclassdef.n_extern_code_block.language.compile_extern_class(nclassdef.n_extern_code_block.as(not null), nclassdef, ffi_ccu, self)
			end
		end
	end
...
redef class AExternPropdef
	...
	do
		...
		n_extern_code_block.language.compile_extern_method(n_extern_code_block.as(not null), self, amodule.ffi_ccu.as(not null), amodule)
	end
end

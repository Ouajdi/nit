...
class GlobalCompiler
	...
	do
		assert self.runtime_type_analysis.live_types.has(mtype)
		#assert mtype.ctype != "val*" or mtype.mclass.name == "Pointer"
		var v = self.new_visitor
...
class GlobalCompilerVisitor
	...
	do
		if mtype isa MClassType and mtype.mclass.kind == extern_kind and
		   mtype.mclass.name != "NativeString" then
			...
			#self.add("{res}->classid = &class_{mtype.c_name};")
			return res
		else
			return value
		end
	...
	do
		...
		if m.is_extern then recv = unbox(recv, recv_type)
	...
	redef fun unbox_signature(m, args)
	do
		var recv = args.first
		for i in [0..m.msignature.arity[ do
			var t = m.msignature.mparameters[i].mtype
			...
			t = self.resolve_for(t, recv)
			if m.is_extern then args[i+1] = self.unbox(args[i+1], t)
		end
	end

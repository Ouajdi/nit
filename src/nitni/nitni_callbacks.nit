...
# Native interface services related to callbacks (used underneath the FFI)
...
redef class AExternPropdef
	...
	do
		...
		for sup in foreign_callbacks.supers do v.analysis.add_super_send(sup.from.mclassdef.mclass.mclass_type, sup.from.as(MMethodDef))
...
# Represent prossible calls from C explictly declared
class MExplicitCall
	super NitniCallback
	...
	do
		...
		if mproperty isa MMethod then
			...
			var creturn_type
			if mproperty.is_init then
				creturn_type = recv_mtype.cname_blind
			else if signature.return_mtype != null then
				var ret_mtype = signature.return_mtype
				ret_mtype = ret_mtype.resolve_for(recv_mtype, recv_mtype, from_mmodule, true)
				creturn_type = ret_mtype.cname_blind
			else
				creturn_type = "void"
			end
			...
			var cparams = new List[String]
			if not mproperty.is_init then
				cparams.add( "{recv_mtype.cname_blind} self" )
			end
			for p in signature.mparameters do
				var param_mtype = p.mtype.resolve_for(recv_mtype, recv_mtype, from_mmodule, true)
				cparams.add( "{param_mtype.cname_blind} {p.name}" )
			end
...
class MExplicitSuper
	...
	# Signature of this call in C as seen by user
	# TODO move to common_ffi::c
	fun csignature: String
	do
		var mproperty = self.from.mproperty
		...
		if signature.return_mtype != null then
			creturn_type = signature.return_mtype.cname
		else
			creturn_type = "void"
		end

		var cname = "{from.mclassdef.mclass.name}_{mproperty.short_cname}___super"

		var cparams = new List[String]
		cparams.add( "{from.mclassdef.mclass.mclass_type.cname} self" )
		for p in signature.mparameters do
			cparams.add( "{p.mtype.cname} {p.name}" )
		end
...
class MExplicitCast
	...
	fun check_csignature: String
	do
		return "int {check_cname}({from.cname} from)"
	end

	fun check_csignature_blind: String
	do
		return "int {check_cname}({from.cname_blind} from)"
	end

	fun cast_cname: String do return "{from.mangled_cname}_as_{to.mangled_cname}"

	fun cast_csignature: String
	do
		return "{to.cname} {cast_cname}({from.cname} from)"
	end

	fun cast_csignature_blind: String
	do
		return "{to.cname_blind} {cast_cname}({from.cname_blind} from)"
	end

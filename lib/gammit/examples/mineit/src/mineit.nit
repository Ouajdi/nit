...
class MineitWorld
	...
	do
		...
		# mossy
		var block = new Block(0.0, 0.0, 0.0)
		block.texture = texture.subtexture(tile_size*4, tile_size*2, tile_size, tile_size)
		app.add block

		# planks
		block = new Block(1.0, 0.0, 0.0)
		block.texture = texture.subtexture(tile_size*4, tile_size*0, tile_size, tile_size)
		app.add block

		# cage
		block = new Block(0.0, 1.0, 0.0)
		block.texture = texture.subtexture(tile_size*1, tile_size*4, tile_size, tile_size)
		...
		block.texture = texture.subtexture(tile_size*1, tile_size*3, tile_size, tile_size)
		block.is_opaque = false
		# Use metal instead to look prettier with optimized
		#block.texture = texture.subtexture(tile_size*6, tile_size*1, tile_size, tile_size)
		app.add block

		# diamond
		block = new Block(1.0, 0.0, 1.0)
		block.texture = texture.subtexture(tile_size*8, tile_size*1, tile_size, tile_size)
		app.add block
		...
		block = new Block(-100.0, 30.0, 0.0)
...
redef class GammitApp
	...
	#var placable_tile: Texture = texture.subtexture(tile_size*6, tile_size*0, tile_size, tile_size) is lazy
	var placable_tile: Texture = texture.subtexture(tile_size*7, tile_size*0, tile_size, tile_size) is lazy
	...
	do
		...
		display.background_color = new Color(0.0, 157.0/255.0, 249.0/256.0, 1.0)
	...
	do
		...
		var last_position = new Point3d[Float].from(camera.position)
	...
	do
		...
		var x = camera.position.x.round
		var y = camera.position.y
		var z = camera.position.z.round

		var block_over = world.blocks[x, y.round+1.0, z]
		var block_head = world.blocks[x, y.round, z]
		var block_feet = world.blocks[x, y.round-1.0, z]
		var block_under = world.blocks[x, y.round-2.0, z]

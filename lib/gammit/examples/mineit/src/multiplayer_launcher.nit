...
class MineitLauncher
	...
	private var gammit_app: nullable GammitApp = null

	redef fun signal(sender, op)
	do
		if sender == but_host then
			gammit_app = new MineitServer
		else if sender == but_join then
			gammit_app = new MineitClient
		else abort

		gammit_app.server_address = ""+txt_server.text
		gammit_app.server_port = txt_port.text.to_i
...
# Once the launcher is claused, launch gammit
var gammit_app = gtk_app.gammit_app

gammit_app.setup
gammit_app.setup_network
gammit_app.run

...
# Main threads support and intro the classes `Thread`, `Mutex` and `Barrier`
module threads is
...
class Thread
	...
	redef fun ==(other)
	do
		if not other isa Thread then return false
		var other_native = other.native
		return (native == null and other_native == null and super) or
			(native != null and other_native != null and native.equal(other_native))
	end
...
fun check_cancel `{ pthread_testcancel(); `}

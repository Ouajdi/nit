	...
	init do
		...
		function modify_people(ele, id){
			if (in_modification_id != null) {
				...
				$('#delete_'+in_modification_id).prop("disabled", true);
			}
...
"""
		...
		for i in participants(db) do
			...
			t.add """<button class="btn btn-xs btn-danger" type="button" onclick="remove_people(this)" id="delete_{{{i.id}}}" disabled=true>Delete</button></center></td>"""
			t.add "</tr>"
		end
		t.add """
